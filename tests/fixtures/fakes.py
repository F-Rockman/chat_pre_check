...
class FakeDeviceResolver:
    def resolve(self, text: str, topk: int = 5) -> list[Candidate]:
        ...
        if "设备a" in text:
            return [
                Candidate("dev_a", "设备A", 0.88),
                Candidate("dev_b", "设备B", 0.60),
            ][:topk]
        ...
        if "olt-01" in text:
            return [
                Candidate("dev_olt_01", "OLT-01", 0.92),
                Candidate("dev_olt_02", "OLT-02", 0.71),
            ][:topk]
        return []


class FakeRegionResolver:
    def resolve(self, text: str, topk: int = 5) -> list[Candidate]:
        mapping = {
            "广州": ("region_gz", "广州"),
            "北京": ("region_bj", "北京"),
...
class FakeVectorRetriever:
    def search_scene(self, query_text: str, topk: int = 5) -> list[SearchHit]:
        ...
        elif any(k in text for k in ["丢包", "时延", "延迟", "抖动", "带宽", "吞吐", "p95", "相关性", "占比", "环比", "同比"]):
            hits = [
                SearchHit("scene_alarm_analysis", 0.95, {"scene_id": "alarm.analysis"}),
                SearchHit("scene_ticket_analysis", 0.60, {"scene_id": "ticket.analysis"}),
            ]
        elif any(k in text for k in ["cpu", "内存", "离线", "设备", "健康分", "利用率"]):
            hits = [
                SearchHit("scene_device_query", 0.95, {"scene_id": "device.query"}),
                SearchHit("scene_alarm_query", 0.70, {"scene_id": "alarm.query"}),
            ]
        elif any(k in text for k in ["设备", "离线", "健康分"]):
            hits = [
                SearchHit("scene_device_query", 0.95, {"scene_id": "device.query"}),
                SearchHit("scene_alarm_query", 0.70, {"scene_id": "alarm.query"}),
            ]
        elif any(k in text for k in ["关联率", "占比", "相关性", "同比", "环比", "p95"]):
            hits = [
                SearchHit("scene_alarm_analysis", 0.94, {"scene_id": "alarm.analysis"}),
                SearchHit("scene_ticket_analysis", 0.60, {"scene_id": "ticket.analysis"}),
            ]
        elif "工单" in text:
            hits = [
