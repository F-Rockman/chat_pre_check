...
def test_extract_slots_for_interface_topn_query():
    slots = ENGINE.extract_slots("alarm.interface.error.topn", normalize_text("过去24小时接口错包告警前10名"))
...
def test_extract_slots_for_count_query():
    slots = ENGINE.extract_slots("device.offline.count", normalize_text("昨天华东离线设备有多少"))
...
def test_extract_slots_for_alarm_count_query():
    slots = ENGINE.extract_slots("alarm.critical.count", normalize_text("今天广州严重告警数量"))
...
def test_extract_slots_for_cpu_threshold_list_query():
    slots = ENGINE.extract_slots("device.cpu.over.list", normalize_text("查询最近cpu大于80的设备列表"))
...
def test_extract_slots_tolerates_common_cpu_and_device_typos():
    slots = ENGINE.extract_slots("device.cpu.over.list", normalize_text("查询cup大余88的设别列表"))
    assert slots["entity_type"] == "device"
    assert slots["metric"] == "cpu_usage"
    assert slots["query_operator"] == "list"
